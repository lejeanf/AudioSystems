...
public class MixerManager : MonoBehaviour
{
    ...
    private static async UniTask WaitForGate(string gate, Func<bool> condition, float timeout, CancellationToken token)
    {
        ...
        if (condition())
        {
           if (isDebug) Debug.Log($"[MixerManager] unmute gate '{gate}' passed after {Time.unscaledTime - start:F1}s");
        }
        else
        {
          if (isDebug) Debug.LogWarning($"[MixerManager] unmute gate '{gate}' timed out after {timeout}s - " +
